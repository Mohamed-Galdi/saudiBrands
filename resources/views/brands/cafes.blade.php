        ...
        <div class="grid w-full grid-cols-3 justify-items-center gap-12 py-12 ">
            @forelse ($cafes as $cafe)
                <div
                    ...
                    <img src={{ asset('storage/' . $cafe->logo) }} alt="" class="h-2/3 object-cover">
                    <p class="font-pr text-xl text-center mt-2">{{ $cafe->name }}</p>
                    <p class="text-pr_dark underline font-pr text-center">{{ $cafe->address }}</p>
                    <div class="w-full my-4 flex justify-center items-start gap-8">
                        <div class=" flex items-center justify-center cursor-pointer">
                            <form action="{{ route('addToFavorites', ['brand' => $cafe->id]) }}" method="POST">
                                @csrf
                                <button type="submit" 
                        ...
                        <div class=" flex items-center justify-center cursor-pointer">
                            <a href={{ 'https://' . $cafe->address }} target="_blank"
